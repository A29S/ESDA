...
{
	/* complexidade do algoritmo: __O(log n)?____ */
if(no==NULL)
	return 0;	

int count=0, right, left=0;

	if(no->str[0]==inicio)
		count++;

right=procura_inicio(no->direita,inicio);

left=procura_inicio(no->esquerda,inicio);
