...
lista* filtra_titulos(lista *lista1, lista *lista2)
{
	...
	if(lista1==NULL||lista2==NULL||lista_tamanho(lista1)==0)
		return NULL;
...
aux=lista1->inicio;
	...
	if(lista_insere(nova,aux->str,NULL)==NULL){	
		...
		return NULL;}
	aux=aux->proximo;}
if(lista_tamanho(lista2)!=0){
aux=lista2->inicio;
	
	while(aux!=NULL){
	lista_remove(nova,lista_pesquisa(nova,aux->str,LISTA_INICIO));
 ...
 aux=lst->inicio;
 	...
 	while(aux!=NULL){
 	  for(int i=0; i<strlen(inicio); i++){
 	   	if(aux->str[i]==inicio[i])
 	   	  count++;}
 	if(count==strlen(inicio)){
 	   aux=lista_remove(lst,aux);
 	   total++;
 	   count=0;
 	   if(aux==NULL)
 	    return total;
...
aux=p->raiz;
    ...
    while(aux!=NULL){
		if(strcmp(aux->elemento,titulo)==0)
			return 0;
